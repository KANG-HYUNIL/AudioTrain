...
class Block(nn.Module):
    def __init__(
        ...
        # If repconv_cfg is provided and enabled, use RepConvBlock for the depthwise conv
        if repconv_cfg is not None and repconv_cfg.get("enable", False) and RepConvBlock is not None:
            # RepConvBlock wraps conv+bn+relu; set groups=exp_channels to create depthwise behavior
            branches = repconv_cfg.get("branches", None)
            depth_conv = RepConvBlock(exp_channels, exp_channels, kernel_size=3, stride=stride,
                                      padding=1, groups=exp_channels, bias=False, branches=branches)
        else:
...
class Network(nn.Module):
    def __init__(self, config):
        ...
        repconv_cfg = config.get('repconv_cfg', None)
...
def get_model(n_classes=10, in_channels=1, base_channels=32, channels_multiplier=2.3, expansion_rate=3.0,
              n_blocks=(3, 2, 1), strides=None, repconv_cfg=None):
    """
    ...
    if repconv_cfg is not None:
        model_config['repconv_cfg'] = repconv_cfg
    # repconv_cfg can be provided via model_config['repconv_cfg'] if present
    # keep backward compatibility: if callers pass extra key repconv_cfg, Network will ignore it unless provided
