...
class PLModule(pl.LightningModule):
    def __init__(self, config):
        ...
        # 모델 로딩: student_cfg/teacher_cfg가 있으면 해당 모델 인스턴스 생성
        repconv_cfg = getattr(config, 'repconv', None)
        if hasattr(config, 'student_cfg') and config.student_cfg:
            self.model = get_student_model(config.student_cfg)
        else:
        ...
        # teacher 모델은 KD 등에서 사용 예정 (필요시 self.teacher = get_teacher_model(config.teacher_cfg))
